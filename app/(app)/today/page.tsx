...
import type { Task, Learning, DailyLog } from "@/types";
...
export default function TodayPage() {
  ...
  const load = async () => {
    const [tr, lr, logr] = await Promise.all([
      ...
      fetch("/api/learnings?limit=1").then((r) => r.json()),
  ...
  const handleArchive = (id: string) => setTasks((t) => t.filter((task) => task.id !== id));
  ...
  const handleEnergySet = async (val: number) => {
    ...
    await fetch("/api/logs", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ date: format(new Date(), "yyyy-MM-dd"), energyLevel: val, mood: todayLog?.mood || 3 }),
    });
  };
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8">
      <div className="mb-8">
        <p className="text-xs text-[#666] uppercase tracking-widest mb-1">{format(new Date(), "EEEE")}</p>
        <h1 className="text-2xl font-semibold text-[#ededed]">{format(new Date(), "MMMM d, yyyy")}</h1>
      </div>

      <div className="mb-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-[#ededed] uppercase tracking-wider">Energy Check-in</h2>
          <span className="text-xs text-[#666]">{energy > 0 ? `${energy}/5` : "not set"}</span>
        </div>
          ...
          {[1, 2, 3, 4, 5].map((v) => (
            <button
              ...
              className={`w-8 h-8 rounded-full border transition-all text-xs font-medium ${
                v <= energy ? "bg-accent border-accent text-black" : "border-[#2a2a2a] text-[#666] hover:border-accent/50"
              }`}
            >
              {v}
            </button>
      ...
      <div className="bg-[#111111] border border-[#1f1f1f] rounded-xl p-5 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setAddOpen(true)} className="flex items-center gap-1.5 text-xs text-accent hover:text-accent/80 font-medium">
            <Plus size={13} /> Add
          </button>
        </div>

        {tasks.length === 0 ? (
          <p className="text-sm text-[#666] text-center py-8">No tasks for today. Add something.</p>
        ) : (
          tasks.map((t) => <TaskItem key={t.id} task={t} onArchive={handleArchive} />)
        )}
      </div>
      ...
      {latestLearning && (
        <div className="bg-[#111111] border border-[#1f1f1f] rounded-xl p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            ...
            <a href="/learn" className="flex items-center gap-1 text-xs text-[#666] hover:text-accent">
              vault <ChevronRight size={12} />
            </a>
          </div>
          <p className="text-sm text-[#c8e6b0] leading-relaxed">{latestLearning.claudeSummary}</p>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Add Task">
        <form onSubmit={handleAddTask} className="flex flex-col gap-4">
          <div>
            ...
            <input value={newTask.title} onChange={(e) => setNewTask({ ...newTask, title: e.target.value })} className={inputClass} placeholder="What needs to be done?" required />
          </div>
          ...
          <div>
            ...
            <input value={newTask.tags} onChange={(e) => setNewTask({ ...newTask, tags: e.target.value })} className={inputClass} placeholder="react, backend, review" />
          </div>
          <button type="submit" disabled={saving} className="bg-accent text-black text-sm font-semibold rounded-lg py-2.5 hover:bg-accent/90 transition-colors disabled:opacity-50">
